...
async function hasCacheDataEntries(cacheDir: string): Promise<boolean> {
  try {
    const files = await fs.readdir(path.join(cacheDir, 'data'));
...
describe('config merge order', () => {
  test('options override YAML env which overrides YAML root', async () => {
    await withTempCwd(async (dir) => {
      ...
      expect(await hasCacheDataEntries(path.resolve(dir, '.cache/opts'))).toBe(true);
      expect(await hasCacheDataEntries(path.resolve(dir, '.cache/env'))).toBe(false);
      expect(await hasCacheDataEntries(path.resolve(dir, '.cache/root'))).toBe(false);
    });
  });
  ...
  test('ENV overrides everything: FASTMD_DISABLE and FASTMD_CACHE_DIR', async () => {
    await withTempCwd(async (dir) => {
      ...
      try {
        ...
        expect(await hasCacheDataEntries(path.resolve(dir, '.cache/envvar'))).toBe(false);
        expect(await hasCacheDataEntries(path.resolve(dir, '.cache/opts'))).toBe(false);
        expect(await hasCacheDataEntries(path.resolve(dir, '.cache/root'))).toBe(false);
      } finally {
  ...
  test('YAML env[NODE_ENV] overrides YAML root when no options/env', async () => {
    await withTempCwd(async (dir) => {
      ...
      expect(await hasCacheDataEntries(path.resolve(dir, '.cache/env'))).toBe(true);
      expect(await hasCacheDataEntries(path.resolve(dir, '.cache/root'))).toBe(false);
    });
  });
});
